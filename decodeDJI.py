...
def OSD(payload):
	...
	DJIFrame['OSD'].append([len(payload),longitude*180/myPI,latitude*180/myPI,height*0.1,xSpeed*0.1,ySpeed*0.1,zSpeed*0.1,pitch*0.1,roll*0.1,yaw*0.1,
...
def Home(payload):
	...
	DJIFrame['Home'].append([len(payload),longitude*180/myPI,latitude*180/myPI,height*0.1,
...
def CUSTOM(payload):
	...
	if updateTime > 17000991717187400:
		...
		DJIFrame['CUSTOM'].append([len(payload),hSpeed,distance,datetime.utcfromtimestamp(updateTime/1000).strftime('%Y-%m-%d %H:%M:%S')])
...
def RECOVER(payload):
	...
	DJIFrame['RECOVER'].append([len(payload),droneType,appType,(appVersionH<<16)|(appVersionM<<8)|appVersionL,aircraftSn.decode('utf-8',errors='ignore'),aircraftName.decode('utf-8',errors='ignore'),datetime.utcfromtimestamp(activeTimestamp).strftime('%Y-%m-%d %H:%M:%S'),cameraSn.decode('utf-8',errors='ignore'),rcSn.decode('utf-8',errors='ignore'),batterySn.decode('utf-8',errors='ignore')])
 ...
 #  here total : 143 bytes
	...
	if len(DJIFrame['Detail']) == 0:
		...
		DJIFrame['Detail'].append([cityPart.decode('utf-8',errors='ignore'),street.decode('utf-8',errors='ignore'),city.decode('utf-8',errors='ignore'),area.decode('utf-8',errors='ignore'),
			...
			datetime.utcfromtimestamp(timestamp/1000).strftime('%Y-%m-%d %H:%M:%S'),longitude,latitude,totalDistance,totalTime/1000,maxHeight,maxHorizontalSpeed,maxVerticalSpeed,
			...
			appType,'.'.join([str(x) for x in appVersion]),datetime.utcfromtimestamp(activeTimestamp/1000).strftime('%Y-%m-%d %H:%M:%S')])
